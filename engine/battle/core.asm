...
.wild
	...
	ld [hl], a
	call LegendaryCheck
	ret nz ; legendary detected, skip exp calculation
	hlCoord 12, 5
	deCoord 12, 6
...
TrainerBattleVictory: ; 3c696 (f:4696)
	call Func_3c643
	callba PlayTrainerVictoryMusic
...
.noLinkBattle
	...
	ld [wd732], a
	call ClearScreen
	ld hl, wRanFlag ; reset rematch battle flag
	res 7, [hl]
	scf
	ret
...
.playSound
	...
	call PrintText
	call WaitForSoundToFinish
	call LegendaryCheck ; Say that legendary mon run away too
	call SaveScreenTilesToBuffer1
	scf ; set carry
	ret
...
; can't use items in link battles and battle tent
.battletent
...
; wd033-wd034 contain pointer to pic
	...
	ld a, [W_ISLINKBATTLE]
	and a
	ld a, Bank(RedPicFront) ; Everybody is red in link battle, not necessarily
	jr nz, .loadSprite
	ld a, [W_TRAINERCLASS]
	cp LEAGUES_PC
	jr nz, .notgiosim
	ld a, Bank(LeaguePCPic)
	jr z, .loadSprite
.notgiosim
	cp TPPRED
